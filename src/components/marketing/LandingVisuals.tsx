...
export function ConnectedEcosystem() {
  return <ChapterMotion name="ecosystem" className={s.ecosystem} duration={6000}>
    ...
    <div className={s.activeEcosystem}>
      ...
      <svg className={s.ecosystemLines} viewBox="0 0 1000 132" preserveAspectRatio="none" aria-hidden="true"><g fill="none" stroke="currentColor" strokeWidth="1">{sourcePaths.map((path,i)=><path key={path} d={path} data-source-route={i} />)}</g>{["Cerere client","Întâlnire · 03 sept.","Document selectat","Caz · Atelier Nord","Promisiune · 04 sept."].map((label,i)=><g key={label} data-source-packet={i} opacity="0" className={s.informationPacket}><rect x="-77" y="-14" width="154" height="28" rx="5"/><text textAnchor="middle" dominantBaseline="central">{label}</text></g>)}</svg>
      <div className={s.contextCore} data-enter="6"><Image src="/marketing/revenew-r.png" width={44} height={44} alt="" /><div><strong>Atelier Nord</strong><p>ReveNew · revenire de verificat.</p></div><span><ShieldCheckIcon aria-hidden="true" />Surse permise</span></div>
    ...
    <div className={s.evaluationField}><div className={s.evaluationHeading}><div><span className={s.miniLabel}>Evaluare la cerere</span><h3>Ai un alt sistem? Pornim de la el.</h3></div><a href="/solicita-demo">Discută cu noi</a></div><SystemDisclosure><div className={s.providerGroups}>{evaluationGroups.map(group=><div key={group.label} className={s.providerGroup}><h4>{group.label}</h4><div className={s.providerField}>{group.providers.map(provider=><div key={provider.name} data-wordmark={"wordmark" in provider}><Image src={provider.src} width={48} height={32} alt={"wordmark" in provider?provider.name:""} /><span>{!("wordmark" in provider)&&<strong>{provider.name}</strong>}{"detail" in provider&&<small>{provider.detail}</small>}</span></div>)}</div></div>)}</div></SystemDisclosure><div className={s.billingEvaluation}><span>Business / România</span><Image src="/marketing/providers/smartbill.png" width={152} height={40} alt="SmartBill" /><small>Evaluare de integrare la cerere</small></div><p>Nu îți cerem să reconstruiești procesul de la zero. Evaluăm integrarea în funcție de flux, acces și datele disponibile.</p><details className={s.integrationDetails}><summary>Disponibilitate și mărci</summary><p>Sistemele din această zonă sunt propuneri pentru evaluare; nu au un conector ReveNew implementat. Mărcile identifică instrumentele discutate, fără a indica un parteneriat.</p></details></div>
  </ChapterMotion>;
}
...
export function TrustArchitecture() {
  return <ChapterMotion name="trust" className={s.trustArchitecture} duration={6800}>
    <div className={s.trustBoundaries}><span className={s.trustSignalTrack} aria-hidden="true"><i className={s.trustSignal} /></span><div className={s.inputLane}><AccessRole /></div>
      <div className={s.authorizedZone}><div className={s.boundaryLabel}><LockClosedIcon aria-hidden="true" />Limita spațiului de lucru<span>Izolare între companii</span></div><div className={s.permittedSources}><span className={s.miniLabel}>SURSE PERMISE PENTRU CAZ</span><div className={s.trustSources}><Image src="/brands/google/gmail.svg" width={24} height={24} alt="Gmail" /><Image src="/brands/applications/google-drive.svg" width={24} height={24} alt="Drive" /><ExcelMark /></div><p>Conversația selectată · documentul · intervalul citat</p></div><div className={s.intelligenceLane}><div data-enter="3"><DocumentTextIcon aria-hidden="true" /><h3>Pipeline.xlsx</h3><p>Oferte · A8:F8 · versiunea 3</p></div><ArrowRightIcon aria-hidden="true" /><div data-enter="4"><SparklesIcon aria-hidden="true" /><h3>Revenire de verificat</h3><p>Recomandare pentru Atelier Nord</p></div></div><div className={s.preparedLane} data-enter="5"><span className={s.statusDot} /><strong>Revenire comercială pregătită</strong><span>Editabil · neexecutat</span></div></div>
      <div className={s.authorityLane} data-enter="6"><div className={s.humanGate}><ShieldCheckIcon aria-hidden="true" /><span className={s.miniLabel}>Autoritatea finală</span><h3>Decizia<br />umană.</h3><p>Fluxul automat se oprește.</p><div><PersonAvatar /><span>Ana Popescu</span><LockClosedIcon aria-hidden="true" /></div></div><div className={s.auditReceipt}><DocumentTextIcon aria-hidden="true" /><span><strong>Audit · revizuire deschisă</strong><small>Ana Popescu · 06 sept., 14:35<br/>Pipeline.xlsx · v3 · A8:F8</small></span></div></div>
