 ...
 return <ChapterMotion name="workflow" className={s.workflow} duration={10800}>
  ...
  <div className={s.engineBody}><div className={s.engineCanvas}>
   ...
   <div ref={graph} className={s.engineGraph}>
    ...
    {steps.map((step,i)=><div className={s.engineNode} data-flow-node={i} data-flow-state={i===4?"review":"complete"} key={step.title}><i className={s.enginePortIn}/><step.icon aria-hidden="true"/><div><small>0{i+1} / {["SEMNAL","DOVADĂ","CONDIȚIE","PAS PREGĂTIT","REVIZUIRE"][i]}</small><h3><button type="button" onClick={()=>inspect(i)} aria-label={`Inspectează etapa ${i+1}: ${step.title}`}>{step.title}</button></h3><p>{step.detail}</p><span className={s.nodeOutput}>{i===0?"Termen → semnal":i===1?"Sursă → fapt citat":i===2?"Pas absent → clarificare":i===3?"Context → propunere":"Propunere → decizie"}</span></div><i className={s.enginePortOut}/></div>)}
    <div className={s.clarificationBranch} data-clarification-node><span>Dacă sursele nu sunt suficiente</span><strong>Cere clarificări</strong><small>Pregătirea rămâne oprită.</small></div>
   </div>
