...
export function LandingChapters() {
  return <div className={s.chapters}>
    ...
    <section id="securitate" className={`${s.chapter} ${s.deep}`} aria-labelledby="trust-title"><div className={s.container}>
      <div className={s.heading}><div><p className={s.overline}>06 / Arhitectura controlului</p><h2 id="trust-title">Controlul rămâne<br />în compania ta.</h2></div><p className={s.lead}>Cine vede informația, cine aprobă și pe ce dovezi se bazează decizia rămân explicite.</p></div><TrustArchitecture />
    </div></section>
    <section className={`${s.chapter} ${s.light}`} aria-labelledby="fit-title"><div className={`${s.container} ${s.fitLayout}`}><div><p className={s.overline}>07 / Pentru ce fel de echipe</p><h2 id="fit-title">Pentru echipe în care o ofertă<br />trece prin mai multe mâini.</h2><p className={s.lead}>De la revenirea comercială la decizia operațională, fiecare predare are nevoie de continuitate.</p><div className={s.processRibbon}><span>Comercial</span><ArrowRightIcon aria-hidden="true" /><span>Tehnic</span><ArrowRightIcon aria-hidden="true" /><span>Decizie</span></div><p className={s.industries}>Servicii B2B · distribuție · logistică · echipamente<br />Construcții · mentenanță · software · consultanță</p></div><div className={s.fitList}>
