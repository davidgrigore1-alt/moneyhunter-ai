...
export function CommercialTruthSnapshot({truth,onPrepare,compact=false,showFacts=true,prepareLabel="Pregătește următorul pas"}:{truth:CommercialTruth|null;onPrepare?:()=>void;compact?:boolean;showFacts?:boolean;prepareLabel?:string}){
 ...
 const reviewedIntegrityKeys=new Set((truth.contextIntegrityPersistence?.cases??[])
  .filter(item=>item.state==="resolved"||item.state==="dismissed").map(item=>item.findingKey));
 const visibleIssues=truth.issues.filter(item=>!(item.origin==="context_integrity"&&item.integrityFindingKey&&reviewedIntegrityKeys.has(item.integrityFindingKey)));
 const discrepancies=visibleIssues.filter(item=>item.kind==="interpretation").length;
 const integrityCount=truth.contextIntegrity?.findings.length??0;
 const hasReviewedCurrentIntegrity=integrityCount>0&&truth.issues.some(item=>item.origin==="context_integrity"&&!!item.integrityFindingKey&&reviewedIntegrityKeys.has(item.integrityFindingKey));
 const displayStateLabel=!visibleIssues.length&&hasReviewedCurrentIntegrity?"Decizie consemnată":truthStateLabels[truth.state];
 return <section aria-label="Situație comercială verificabilă" className="min-w-0 border-y border-[rgb(var(--border))] py-3">
  <div className="flex items-center justify-between gap-3">
   ...
   <span className={"text-xs "+(truth.state==="confirmed"?"text-[rgb(var(--success-text))]":"text-[rgb(var(--text-muted))]")}>{discrepancies?discrepancies+" "+(discrepancies===1?"neconcordanță":"neconcordanțe"):displayStateLabel}</span>
  </div>
  ...
  <details className="mt-2">
   ...
   <div className="mt-2 divide-y divide-[rgb(var(--border))]">
    {visibleIssues.slice(0,4).map(item=><article key={item.id} className="py-3 text-xs leading-5">
