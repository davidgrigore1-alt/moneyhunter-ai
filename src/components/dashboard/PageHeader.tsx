...
export function PageHeader({ entity, eyebrow, title, description, children }: PageHeaderProps) {
  return (
    <div className="flex min-h-14 flex-col justify-center gap-3 border-b border-[rgb(var(--border-subtle))] pb-4 md:flex-row md:items-end md:justify-between">
      <div className="min-w-0 max-w-3xl">
        <p className="text-micro font-semibold tracking-[0.01em] text-[rgb(var(--text-muted))]">{entity ? <span className="mb-2 inline-flex items-center gap-2"><EntityMark entity={entity} />{eyebrow}</span> : eyebrow}</p>
