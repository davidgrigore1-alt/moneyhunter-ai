...
const filterLabels: Record<string, Record<string, string>> = {
  status: { reviewed: "Revizuită", contacted: "Contactată", follow_up_needed: "Follow-up" },
...
export function OpportunityFilters({ filters, savedViews }: { filters: OpportunityFilterState; savedViews?: ReactNode }) {
  ...
  return <div><form id="opportunity-registry-filters" method="get" className="grid gap-2">
    <div className="grid grid-cols-2 items-end gap-2 xl:grid-cols-[minmax(16rem,1fr)_11rem_11rem_12rem_auto]">
      <label className="col-span-2 min-w-0 text-xs font-semibold text-[rgb(var(--text-secondary))] sm:col-span-1">Caută
      ...
      <FilterSelect compact name="status" label="Etapă" value={filters.status} options={[["", "Toate etapele"], ["reviewed", "Revizuită"], ["contacted", "Contactată"], ["follow_up_needed", "Follow-up"]]} />
      ...
      <FilterSelect compact name="sort" label="Ordonare" value={filters.sort} options={[["updated", "Actualizate recent"], ["value", "Valoare · pe monedă"], ["attention", "Prioritate operațională"]]} />
      <div className="flex min-h-9 shrink-0 snap-start items-center justify-end gap-1.5 md:min-w-0">
        ...
        <Button type="submit" variant="secondary" size="small" className="min-h-9">Aplică</Button>
      </div>
    </div>
    {activeFilters.length ? <div className="flex flex-wrap items-center gap-1.5" aria-label="Filtre active">
    ...
    </form><div className="mt-1 grid items-start gap-x-5 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]"><details className="group py-2" open={Boolean(filters.lifecycle || filters.due || filters.contact || filters.decisionMaker)}>
      <summary className="focus-ring inline-flex min-h-9 cursor-pointer list-none items-center rounded-button px-2 text-xs font-semibold text-[rgb(var(--text-secondary))] marker:hidden hover:bg-[rgb(var(--surface-muted))] hover:text-[rgb(var(--foreground))]">
        ...
        {activeCount ? <span className="ml-2 rounded-full bg-[rgb(var(--primary-muted))] px-2 py-0.5 text-xs text-[rgb(var(--primary))]">{activeCount} active</span> : null}
      ...
      <div className="mt-2 grid gap-2 pl-2 sm:grid-cols-2 lg:grid-cols-4">
