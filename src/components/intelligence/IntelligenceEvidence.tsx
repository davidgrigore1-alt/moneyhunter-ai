...
export function IntelligenceEvidence({answer, inline=false}: {answer:CopilotAnswer; inline?:boolean}) {
  ...
  return <section className={inline?"mt-2":styles.evidence} aria-label={inline?"Dovezile afirmației":"Dovezi și acoperire"}>
    {inline?<div className="flex flex-wrap gap-2">{answer.evidence.map(e=><button key={e.sourceId} type="button" onClick={()=>setSelected(e)} className="focus-ring min-h-9 max-w-full rounded-control border border-[rgb(var(--border-strong))] bg-[rgb(var(--surface-subtle))] px-3 py-1.5 text-left text-xs text-[rgb(var(--text-secondary))] hover:border-[rgb(var(--intelligence))] hover:text-[rgb(var(--foreground))]">{e.label}</button>)}</div>:<details>
    <summary className={`focus-ring ${styles.evidenceSummary}`}><span><DocumentTextIcon aria-hidden="true" />{uniqueEvidenceSources(answer.evidence)} {uniqueEvidenceSources(answer.evidence) === 1 ? "sursă" : "surse"} · {answer.evidence.length} {answer.evidence.length === 1 ? "dovadă" : "dovezi"} · Acoperire și proveniență</span></summary>
