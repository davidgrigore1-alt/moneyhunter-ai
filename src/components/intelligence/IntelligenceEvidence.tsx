...
export function IntelligenceEvidence({answer, inline=false}: {answer:CopilotAnswer; inline?:boolean}) {
  ...
  return <section className={inline?"mt-2":styles.evidence} aria-label={inline?"Dovezile afirmației":"Dovezi și acoperire"}>
    ...
    <ol className="mt-2 divide-y divide-[rgb(var(--border-subtle))]">{answer.evidence.map((e,index)=><li key={e.sourceId}><button type="button" onClick={()=>setSelected(e)} className={`focus-ring ${styles.evidenceRow}`}><span className={styles.evidenceIndex}>{index+1}</span><span className={styles.evidenceRowContent}>{e.label}<small className={styles.evidenceRowMeta}>{e.sourceType} · {e.observedAt ? formatProductDateTime(e.observedAt) : "Data sursei indisponibilă"}</small></span><span className={styles.evidenceInspect}>Inspectează</span></button></li>)}</ol>
