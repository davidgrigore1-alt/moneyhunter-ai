...
function requireQuery<T>(result: { data: T; error?: { message?: string; code?: string } | null }, code: string): T {
  if (result.error) {
    console.error(code, { databaseCode: /^[A-Z0-9]{5,12}$/.test(result.error.code ?? "") ? result.error.code : "unavailable" });
...
async function loadRuntimeContext(businessId: string, profileId: string, targetId: string) {
  ...
  const [opportunityResult, actionResult, approvalResult, emailResult, meetingResult, businessResult, preparedResult] = await Promise.all([
    ...
    client.rpc("workflow_has_pending_approval", { target_business_id: businessId, target_opportunity_id: targetId }),
  ...
  if (opportunity.organization_id) { const organizationResult = await client.rpc("workflow_company_name", { target_business_id: businessId, target_company_id: opportunity.organization_id }); const organization = requireQuery(organizationResult, "workflow_context_load_failed"); company = organization ?? null; }
