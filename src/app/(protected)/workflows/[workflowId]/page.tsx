...
export default async function WorkflowEditorPage({ params, searchParams }: {
  ...
  >
    <nav aria-label="Secțiuni workflow" className={controls.segments}>
    ...
    {query?.tested ? <p role="status" className="mt-3 border-l-2 border-[rgb(var(--primary))] bg-[rgb(var(--primary-soft))] px-3 py-2 text-xs text-[rgb(var(--text-secondary))]">Test înregistrat fără executarea acțiunilor. Rezultatul este disponibil în Rulări.</p> : null}
      ...
      <div className={selectedRun ? "grid xl:grid-cols-[minmax(0,0.9fr)_minmax(28rem,1.1fr)]" : ""}>
        {runs.length ? <div className="divide-y divide-[rgb(var(--border))] xl:border-r xl:border-[rgb(var(--border))]">{runs.map((run) => {
          ...
          return <a key={run.id} href={href} aria-current={selectedRun?.id === run.id ? "true" : undefined} className="focus-ring grid gap-2 px-4 py-3 transition-colors duration-fast hover:bg-[rgb(var(--surface-hover))] aria-[current=true]:bg-[rgb(var(--surface-elevated))] md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
            <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={statusClass(runState.tone)}>{run.is_test_run ? "Test · " : ""}{run.is_test_run && run.status === "prepared" ? "Ar pregăti lucru" : runState.label}</span><span className="truncate text-xs font-semibold">{target?.title || "Oportunitate indisponibilă"}</span></div><p className="mt-1 truncate text-xs text-[rgb(var(--text-muted))]">{guard.label} · {formatUserFacingText(String(run.guard_reason || "Evaluare înregistrată.").replace("TEST RUN · ", ""))}</p></div>
        ...
        })}</div> : <div className="grid place-items-center px-5 py-12 text-center"><div><p className="text-sm font-semibold">Nicio rulare încă</p><p className="mt-2 max-w-sm text-xs leading-5 text-[rgb(var(--text-muted))]">Rulează un test din Editor pentru a verifica logica fără executarea acțiunilor. Rulările automate vor apărea numai pentru declanșatoarele conectate.</p><a href={baseHref} className="focus-ring mt-4 inline-flex h-9 items-center rounded-[8px] border border-[rgb(var(--border))] px-3 text-xs font-semibold hover:bg-[rgb(var(--surface-elevated))]">Deschide Editor</a></div></div>}
        ...
        {selectedRun && trace ? <aside aria-labelledby="run-detail-title" className="bg-[rgb(var(--surface-subtle))] px-5 py-5 xl:max-h-[min(46rem,calc(100dvh-13rem))] xl:overflow-y-auto">
          <div className="flex items-start justify-between gap-3 border-b border-[rgb(var(--border))] pb-4"><div><p className="micro-label">{selectedRun.is_test_run ? "Test fără executare" : "Rulare workflow"}</p><h3 id="run-detail-title" className="mt-1 text-base font-semibold">Detaliul deciziei</h3></div><a href={baseHref + "?view=runs"} aria-label="Închide detaliul rulării" className="focus-ring text-xs font-semibold text-[rgb(var(--text-muted))] hover:text-[rgb(var(--foreground))]">Închide</a></div>
          <DecisionFlow compact steps={[
            { detail: presentWorkflowTrigger(selectedRun.trigger_type as WorkflowTrigger) },
            { detail: selectedRun.is_test_run ? "Test manual pe cazul selectat" : trace.origin, href: "/opportunities/" + selectedRun.target_id, action: "Deschide cazul" },
            { detail: presentGuardDecision(selectedRun.guard_decision).label + " · " + conditionResults(selectedRun.condition_results).filter(item => item.matched).length + " condiții confirmate" },
            { detail: selectedRun.is_test_run ? "Test înregistrat; niciun lucru executat." : idCount(selectedRun.prepared_action_plan_ids) + " planuri pregătite · " + presentWorkflowRunState(selectedRun.status).label },
            { detail: selectedRun.is_test_run ? "Testul nu aprobă și nu activează workflow-ul." : selectedRun.human_approval_required ? "Confirmare necesară pentru planurile protejate." : "Evaluare internă; starea fiecărui efect este păstrată în audit." },
          ]} />
          <details className="mt-4"><summary className="focus-ring cursor-pointer rounded text-xs font-semibold">Detaliile evaluării și audit</summary><ol className="mt-1 divide-y divide-[rgb(var(--border))]">
            <li className="grid grid-cols-[5.5rem_minmax(0,1fr)] gap-4 py-4"><span className="micro-label">01 · Semnal</span><div><p className="text-sm font-semibold">{presentWorkflowTrigger(selectedRun.trigger_type as WorkflowTrigger)}</p><p className="mt-1 text-xs text-[rgb(var(--text-muted))]">{opportunities.find((item) => item.id === selectedRun.target_id)?.title || "Țintă indisponibilă"}</p><p className="mt-1 text-xs text-[rgb(var(--text-muted))]">{selectedRun.is_test_run ? "Test manual pe cazul selectat" : trace.origin}</p>{trace.source ? <><p className="mt-1 text-xs">{formatProductDateTime(trace.source.occurredAt)}</p><p className="mt-1 break-all font-mono text-xs text-[rgb(var(--text-muted))]">Eveniment {trace.source.id}</p>{trace.source.previousStage ? <p className="mt-1 text-xs">{workflowStageLabels[trace.source.previousStage]} → {workflowStageLabels[trace.source.nextStage ?? ""]}</p> : null}</> : <p className="mt-1 text-xs text-[rgb(var(--text-muted))]">{selectedRun.is_test_run ? "Testul nu reprezintă un eveniment comercial produs." : "Sursa istorică nu a fost înregistrată."}</p>}</div></li>
