...
export default async function GovernancePage() {
  ...
  if (!snapshot) return <PageShell wide entity="approvals" eyebrow="Setări" title="Administrare enterprise" description="Guvernanța nu este disponibilă în modul curent." />;
  return (
    <PageShell wide entity="approvals" eyebrow="Setări spațiu de lucru" title="Echipă și guvernanță" description="Acces, atribuiri, aprobări și audit pentru operarea sigură a echipei." actions={<Link href="/settings" className="focus-ring inline-flex min-h-11 items-center rounded-button border border-[rgb(var(--border))] px-4 text-sm font-semibold hover:bg-[rgb(var(--surface-muted))]">Înapoi la setări</Link>}>
      <div className="grid gap-6">
        <nav className={`${controls.segments} sticky top-[4.25rem] z-20 overflow-x-auto`} aria-label="Secțiuni guvernanță">
