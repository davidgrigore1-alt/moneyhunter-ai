...
function groupActions(actions: RecoveryAction[]) {
  ...
  return {
    overdue: actions.filter((action) => action.dueAt && applicationDateKey(new Date(action.dueAt)) < today),
    today: actions.filter((action) => action.dueAt && applicationDateKey(new Date(action.dueAt)) === today),
    next: actions.filter((action) => action.dueAt && applicationDateKey(new Date(action.dueAt)) > today),
...
export default async function TodayPage() {
  ...
  if (!hasAnyActions) {
    return <PageShell wide entity="opportunities" eyebrow="Activitate" title="Activitatea mea" description="Acțiunile atribuite ție, ordonate după termen și prioritate"><div className="grid justify-items-start gap-4"><EmptyState title="Nu ai încă acțiuni atribuite" description="Acțiunile ajung aici după ce deschizi o oportunitate, alegi responsabilul și stabilești următorul pas cu termen sau fără termen." /><Button href="/opportunities">Deschide oportunitățile</Button></div></PageShell>;
  }

  return (
    <PageShell wide entity="opportunities" eyebrow="Sistem de execuție" title="Activitatea mea" description="Ce necesită intervenție, ce este pregătit și unde așteptarea este acțiunea corectă"><div className="grid gap-6">
      <RecordSummaryBar label="Situația acțiunilor mele" items={[
        { label: "Restante", value: groups.overdue.length, tone: "attention" },
        { label: "Scadente astăzi", value: groups.today.length },
        { label: "Fără termen", value: groups.none.length },
        { label: "Finalizate astăzi", value: summary.workQueue.completedToday.length },
      ]} />
